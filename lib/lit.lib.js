...
async function initialisePkp() {
    try {
        ...
        if (!fs.existsSync(path.resolve(__dirname, "../pkp.json"))) {
            const pkpInfo = (await litContracts.pkpNftContractUtils.write.mint()).pkp;
            loggerLib.logInfo(`Minted pkp successfully!`);
            fs.writeFileSync(path.resolve(__dirname, "../pkp.json"), JSON.stringify({
        ...
        const pkpInfo = JSON.parse(fs.readFileSync(path.resolve(__dirname, "../pkp.json"), 'utf8'));
        ...
        if (Number(formattedBalance) < Number(ethers.utils.formatEther(25_000))) {
            ...
            const fundingTx = {
                ...
                gasLimit: 21_000,
                gasPrice: (parseInt(await chainWallet.getGasPrice())*10),
